...
Write-Host "Using R version: $rVersion"
Write-Host "Rscript path: $rscriptPath"
Write-Host "System-wide library path: $systemLibPath"
...
foreach ($package in $Packages) {
    ...
    if ($process.ExitCode -eq 0) {
        Write-Host "Package $package installed or already present in system-wide library." -ForegroundColor Green
    } else {
        Write-Host "Failed to install package $package in system-wide library." -ForegroundColor Red
    }
}
